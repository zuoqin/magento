...
def job(func):
   ...
   *args and **kargs
    ...
    def delay(session, model_name, *args, **kwargs):
        ...
        priority = kwargs.pop('priority', None)
        eta = kwargs.pop('eta', None)
        model_name2 = kwargs.pop('model_name', None)
        max_retries = kwargs.pop('max_retries', None)
        description = kwargs.pop('description', None)

        _logger.info("ZZZZZZZZZZZZZZ      Call job delay")
        print "Arguments are: %s, %s" % (args, kwargs)

        return func(session, model_name, *args, **kwargs)

    _logger.info("ZZZZZZZZZZZZZZ   call job")
    func.delay = delay
    return func
