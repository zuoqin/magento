...
class Worker(threading.Thread):
    ...
    def run_job(self, job):
        ...
        try:
            ...
            if job.eta and job.eta > datetime.now():
                ...
                _logger.info('Wait %s seconds because the delayed '
            ...
            _logger.info('Job:   %s started', job)
            with session_hdl.session() as session:
                job.perform(session)
            _logger.info('Job:   %s done', job)
            ...
            _logger.info('Job:   %s postponed', job)
            ...
            _logger.info('%s OperationalError, postponed', job)
    ...
    def enqueue_job_uuid(self, job_uuid):
        ...
        self.queue.enqueue(job)
        _logger.info('%s enqueued in %s', job, self)
            ...
            with session_hdl.session() as session:
                ...
                try:
                    ...
                    if unicode(err).startswith(no_db_error):
                        _logger.info('Database %s is not an OpenERP database,'
