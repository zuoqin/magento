...
class SaleOrderAdapter(GenericAdapter):
    ...
    def write(self, order_increment, status, comment=None, notify=False):
        _logger.info('start call  sales_order.addComment for order %s with fields: %s',
             order_increment, status)
...
@magento
class SaleCommentExport(ExportSynchronizer):
    _model_name = ['magento.sale.order']



    def _get_data(self, order, fields):        
        result = {}
        if 'status' in fields:
            # result.update({
            #     'status': ORDER_STATUS_MAPPING[order.state]
            # })
            result = ORDER_STATUS_MAPPING[order.state]
        print("start to print SaleCommentExport result:")
        print(result)
        return result

    def run(self, binding_id, fields):
        """ Export the product inventory to Magento """
        order = self.session.browse(self.model._name, binding_id)
        print("order:")
        print(order)
        binder = self.get_binder_for_model()
        print("binder:")
        print(binder)
        magento_id = binder.to_backend(order.id)
        print("magento_id:")
        print(magento_id)
        data = self._get_data(order, fields)
...
def sale_order_modified(session, model_name, record_id, vals):
    #comments - zuoqin
    ...
    _logger.info('ZZZZZZZZZZZ   sale_order_modified')
    print("record_id:")
    print(record_id)
    print("vals:")
    print(vals)

    sale_order = session.browse(model_name, record_id)
    print("sale_order:")
    print(sale_order)


    print("magento_bind_ids:")
    if len(sale_order.magento_bind_ids) < 1:
        return True
    print( sale_order.magento_bind_ids[0] )
    #magento_obj = self.pool.get('magento.sale.order')
    #print("magento_obj:")
    #print(magento_obj)

    magento_order = sale_order.magento_bind_ids[0] #session.browse('magento.sale.order', 8)
    print("magento_order:")
    print(magento_order)
    ...
    print("env:")
    print(env)
    order_commentor = env.get_connector_unit(SaleCommentExport)
    print("order_commentor:")
    #print(order_commentor)
    
    return order_commentor.run(record_id, {'status': 'canceled'})
